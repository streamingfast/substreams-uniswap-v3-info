...
pub fn pool_day_data_create(ops: &mut KvOperations, tx_count_deltas: &Deltas<DeltaBigInt>) {
        ...
        .filter(|d| d.new_value.eq(&BigInt::one()))
    {
        ...
        create_pool_day_data(ops, time_id, &pool_addr, delta.ordinal);
    }
}
...
fn create_pool_day_data(
    ...
    ops.push_new(format!("{pool_addr}:{date_id}:date"), date.to_string(), ordinal);
    ops.push_new(format!("{pool_addr}:{date_id}:volumeUSD"), BigDecimal::zero().to_string(), ordinal);
    ops.push_new(format!("{pool_addr}:{date_id}:tvlUSD"), BigDecimal::zero().to_string(), ordinal);
    ops.push_new(format!("{pool_addr}:{date_id}:feesUSD"), BigDecimal::zero().to_string(), ordinal);
}
...
pub fn swap_volume(ops: &mut KvOperations, swaps_volume_deltas: &Deltas<DeltaBigDecimal>) {
        ...
        ]))
    {
        ...
        ops.push_new(format!("0x{pool_address}:{time_id}:{attribute}"),&delta.new_value.to_string(), delta.ordinal);
    }
}
...
pub fn total_value_locked_usd(ops: &mut KvOperations, derived_tvl_deltas: &Deltas<DeltaBigDecimal>) {
    ...
    {
        let (_, time_id, pool_address) = pool_windows_id_fields(&delta.key);
        ops.push_new(format!("0x{pool_address}:{time_id}:tvlUSD"), &delta.new_value.to_string(), delta.ordinal);
    }
}
